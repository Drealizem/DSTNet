...
class Deblur(nn.Module):    # 本文使用的就是这个模型
    ...
    def forward(self, lrs):
        ...
        tf_wave1_l, tf_wave1_h = self.wave(tf_input_feature)    # 第一次小波变换
        ...
        tf_wave2_l = rearrange(self.transformer_scale4(rearrange(tf_wave2_l, '(b t) c h w -> b t c h w', b=b)), 'b t c h w -> (b t) c h w') # 使用变换域特征融合模块处理第二次小波变换后的低频部分
        ...
        pro_feat = rearrange(self.wave(torch.cat([tf_wave1_l, tf_wave1_h], dim=1), rev=True), '(b t) c h w -> b t c h w', b=b)  # 将传播后的特征与高频部分结合，进行小波逆变换，重建去模糊帧
...
class manual_conv3d_propagation_forward(nn.Module):
    ...
    def forward(self, feature):
        ...
        b, t, c, h, w = feature.size()                          # b t 64 256 256
        forward_list = []   # 初始化一个空列表，用于存储每个时间步的特征
        feat_prop = feature.new_zeros(b, c, h, w)   # 初始化一个特征图，用于存储传播的特征
